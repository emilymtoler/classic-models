...
plot11 = st.columns(1)
with plot11[0]:
    dropbox1 = st.selectbox('Select what you would like to compare', ['quantityOrdered', 'priceDifference', 'expectedProfit', 'profit', 'percentProfit'], key = 'models')
    dropbox1_names = ["Quantity Ordered", "Price Difference", "Expected Profit", "Profit", "Percent Profit"]
    fig, axes = plt.subplots(2,4, sharey=True)
    sns.histplot(ax=axes[0,0], data=PLvc, x=dropbox1)
    plt.xlabel(dropbox1)
    plt.ylabel("Count")
    sns.histplot(ax=axes[0,1], data=PLcc, x=dropbox1)
    sns.histplot(ax=axes[0,2], data=PLtb, x=dropbox1)
    sns.histplot(ax=axes[0,3], data=PLt, x=dropbox1)
...
plot12 = st.columns(1)
with plot12[0]:
    dropbox1 = st.selectbox('Select what you would like to compare', ['quantityOrdered', 'priceDifference', 'expectedProfit', 'profit', 'percentProfit'], key = 'emily')
    dropbox1_names = ["Quantity Ordered", "Price Difference", "Expected Profit", "Profit", "Percent Profit"]
    dropbox2 = st.selectbox('Select what you would like to compare', ['orderDate', 'shippedDate', 'paymentDate'], key = 'hannah')
    dropbox2_names = ['Order Date', 'Shipped Date', 'Payment Date']
    fig, axes = plt.subplots(2,4, sharey=True)
    sns.histplot(ax=axes[0,0], data=PLvc, x=dropbox2, y=dropbox1)
    plt.xlabel(dropbox2)
    plt.ylabel(dropbox1)
    sns.histplot(ax=axes[0,1], data=PLcc, x=dropbox2, y=dropbox1)
    sns.histplot(ax=axes[0,2], data=PLtb, x=dropbox2, y=dropbox1)
    sns.histplot(ax=axes[0,3], data=PLt, x=dropbox2, y=dropbox1)
